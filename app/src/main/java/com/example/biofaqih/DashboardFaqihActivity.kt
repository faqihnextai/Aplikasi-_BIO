...
class DashboardFaqihActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val bottomNav = BottomNavigationView(this).apply {
            ...
            setOnItemSelectedListener {
                when (it.itemId) {
                    R.id.nav_biodata -> switchToFragment(BiodataFragment())
                    R.id.nav_game -> switchToFragment(GameFragment())
                    R.id.nav_contact -> switchToFragment(ContactFragment())
                }
                true
            }
        }
...
class BiodataFragment : Fragment() {
    override fun onCreateView(
        ...
        val imageView = ImageView(context).apply {
            setImageResource(R.drawable.faqih_half)
            layoutParams = FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.MATCH_PARENT
            )
        ...
        menuItems.forEachIndexed { index, (label, value) ->
            val btn = TextView(context).apply {
                text = label
                textSize = if (label == "Nama") 22f else 18f
                setTypeface(null, Typeface.BOLD)
                setTextColor(Color.WHITE)
                gravity = Gravity.CENTER
        ...
        rootLayout.addView(imageView)     // Background
        rootLayout.addView(panelLayout)   // Menimpa gambar
